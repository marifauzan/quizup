...
function SignUp(): JSX.Element {
  ...
  const warningmodal: JSX.Element = (
    <div
      id="alert-1"
      className="flex bg-yellow-100 text-yellow-700 px-3 py-3 rounded relative"
      role="alert"
    >
      <span className="font-medium">Account has already exist. </span> Try Sign
      In.
      <button
        type="button"
  ...
  const addUser = () => {
    axios
      .post("https://616981a909e030001712c409.mockapi.io/users", {
        username: username,
        full_name: fullname,
        email: email,
        password: password,
  ...
  const handleSubmit = async (e: React.SyntheticEvent) => {
    ...
    const userLoggedIn = users.data.find(
      (record: { email: String; password: String }) => record.email === email
    );
      ...
      <div className="h-screen md:flex">
        <div className="flex md:w-1/2 justify-center py-10 items-center bg-white">
          <form className="bg-white" onSubmit={handleSubmit}>
            ...
            <button
              type="submit"
              className="block w-full hover:bg-purple-600 bg-sky-500 mt-4 py-2 rounded-2xl text-white font-semibold mb-2"
            >
              Sign Up
            </button>
            <Link
              to="/Login"
              className="text-sm ml-2 hover:text-rose-800 cursor-pointer underline decoration-blue-500"
            >
