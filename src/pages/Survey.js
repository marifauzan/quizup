        ...
        <div className="flex">
            ...
            <div>
                <div className="pt-20 pl-32 transition-all whitespace-normal w-96">
                    <progress className="progress progress-error w-96 mb-16" value={currentQ} max={questions.length}></progress>
                    <p className="text-gray-500 text-xs h-12">Question {currentQ+1} / {questions.length}</p>
                    <div>
                        <h1 className="text-gray-500 flex-1 text-xl h-16">{questions[currentQ]}</h1>
                        <div className="rating rating-lg mt-12">
                            <input type="radio" id="rate" name="rating" className="mask mask-star-2 bg-rose-400" value={1} onClick={e => setChoice(e.target.value)}/>
                            <input type="radio" id="rate" name="rating" className="mask mask-star-2 bg-rose-400" value={2} onClick={e => setChoice(e.target.value)}/>
                            <input type="radio" id="rate" name="rating" className="mask mask-star-2 bg-rose-400" value={3} onClick={e => setChoice(e.target.value)}/>
                            <input type="radio" id="rate" name="rating" className="mask mask-star-2 bg-rose-400" value={4} onClick={e => setChoice(e.target.value)}/>
                            <input type="radio" id="rate" name="rating" className="mask mask-star-2 bg-rose-400" value={5} onClick={e => setChoice(e.target.value)}/>
                        </div>
                    </div>
                    <button onClick={finish ? undefined : handleNext} className="">
                        <label htmlFor={finish} className="px-6 py-1 bg-rose-500 text-white border-none rounded-xl mt-16 btn modal-button"  
                        disabled={finish}>
            ...
            <div className="absolute inset-y-0 right-0 w-80 bg-white mx-12 my-10 rounded rounded-xl drop-shadow-md grid place-items-center p-8">
            ...
            <label className="modal-box relative" for="">
                    ...
                    <p className="py-4">Thank you for reaching out and providing us with valuable feedback. 
                    Thank you for taking time out of your day to leave us this super kind review.
                    </p>
                    <div className="modal-action">
                        <label for="my-modal-4" className="btn border-none bg-rose-500">Yay!</label>
                    </div>
            </label>
            </label>
